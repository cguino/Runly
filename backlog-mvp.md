...
> Découpage des P0 de la spec v0.2 en epics et stories estimées. Prêt à importer dans Jira/Linear.
...
|---|---|---|
| E0 — Fondations techniques | 21 | — |
| E1 — Onboarding & profil | 18 | E0 |
| E2 — Références physiologiques | 13 | E0 |
| E3 — Moteur de plan périodisé | 21 | E2 |
| E4 — Bibliothèque pédagogique & builder | 16 | E0 |
| E5 — Player de séance | 31 | E0, E4 |
| E6 — Ingestion des séances | 21 | E0 |
| E7 — Charge & jauge ACWR | 18 | E6 |
| E8 — Plan, timeline & flexibilité | 13 | E3 |
| E9 — Notifications & récap hebdo | 8 | E7 |
| E10 — RGPD, réglages & release | 10 | tout |
| **Total** | **~185 pts ≈ 9-10 sprints bruts → 6-7 sprints à 3 devs** | |

---

## E0 — Fondations techniques (21 pts)

- **E0-1** (5) Setup monorepo : app Expo SDK 56 (dev client, TS strict, expo-router 4 tabs), packages `@runly/schemas` (zod) et `@runly/training-engine`, EAS Build + GitHub Actions.
...
- **E0-5** (3) Sentry + PostHog (UE) + plan de tracking des events spec §10 ; demandes externes : quotas Strava, comptes stores.

## E1 — Onboarding & profil (18 pts)
...
- **E1-3** (2) Profil : âge, sexe, poids optionnel, antécédents blessure (type, ancienneté) → flag prudence.
- **E1-4** (2) Contexte : séances/sem (min 2), jours dispo, volume actuel pré-rempli.
- **E1-5** (2) Objectif : distance (5K/10K/semi/marathon), date, ambition, nom épreuve ; garde-fou « objectif irréaliste » (date trop proche vs volume actuel) avec alternatives.
...
- **E3-1** (8) `@runly/training-engine` : génération de plan = fonction pure (profil, objectif, historique) → phases (générale/spécifique/affûtage), semaines allégées 1/3-4, ~80 % EF + 1-2 qualités/sem, progression ≤ 10 % (5-8 % si antécédent), refus < 2 séances/sem. Couverture de tests exhaustive (golden files par persona).
...
## E5 — Player de séance (26 pts) — le module le plus risqué
...
- **E5-2** (5) Tracking : expo-location en foreground service, allure lissée (algo du spike E0-4), distance cumulée, FC si capteur connecté au téléphone.
- **E5-3** (5) UI player conforme charte : timer géant, barre d'allure cible avec curseur, trio stats, coaching (« dans la cible / un poil vite »), prochain bloc.
...
## E6 — Ingestion des séances (21 pts)
...
- **E6-5** (2) Matching séance réalisée ↔ planifiée (même jour ± tolérance, confirmation 1 tap si ambigu).
- **E6-6** (1) Saisie manuelle (durée, distance, RPE) — mode sans montre.

## E7 — Charge & jauge ACWR (18 pts) — le différenciateur

- **E7-1** (3) Calcul de charge : sRPE (RPE × durée) avec fallback durée × zone moyenne ; ACWR 7 j / 28 j ; état « calibration » < 4 semaines ; tests unitaires exhaustifs (trous de données, blessure, reprise).
...
## E8 — Plan, timeline & flexibilité (13 pts)
...
- **E9-2** (5) Récap hebdo (dimanche soir) : réalisé vs prévu, évolution ACWR, message d'adaptation éventuel.

## E10 — RGPD, réglages & release (10 pts)

- **E10-1** (3) Réglages : permissions, notifications, objectif (modifier/abandonner), compte ; suppression de compte = purge complète (RGPD).
- **E10-2** (2) CGU + politique de confidentialité (wording note réglementaire) ; disclaimer sur les écrans de charge.
- **E10-3** (2) Registre des traitements, minimisation validée (agrégats, pas de séries FC brutes), DPA Supabase.
- **E10-4** (3) Release : fiches stores (wording validé), déclarations données santé Apple/Google, vidéo Play si background location, beta TestFlight/Play Internal (50-100 coureurs).
